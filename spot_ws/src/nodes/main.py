...
class Simulation:
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            spot_twist.linear.y = curr_output[0][1].item()
            spot_twist.linear.z = curr_output[0][2].item()
            spot_twist.angular.x = curr_output[0][3].item()
            spot_twist.angular.y = curr_output[0][4].item()
            spot_twist.angular.z = curr_output[0][5].item()

            # Close to goal
            e = ((M.spotPose[0] - goal[0])**2 + (M.spotPose[1] - goal[1])**2 + (M.spotPose[2] - goal[2])**2 ) / 3
            if e < 0.5:
                print("done")
